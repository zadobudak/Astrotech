...
detection_net = cv2.dnn.readNet(join(realpath(dirname(__file__)), 'yolov4_tiny_custom_best.weights'),
                                join(realpath(dirname(__file__)), "yolov4_tiny_custom_test.cfg"))
...
classes = []
with open(join(realpath(dirname(__file__)),"classes.txt"), "r") as file_object:
...
cap = cv2.VideoCapture("last version.mp4")
